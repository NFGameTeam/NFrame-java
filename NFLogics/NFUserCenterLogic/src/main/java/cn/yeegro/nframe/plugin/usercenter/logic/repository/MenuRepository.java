...
//    LinkedHashSet<Menu> findByRoleIdsAndTypeNot(Set<Long> roleIds, int type);
     ...
     * @return /
     */
    @Query(value = "SELECT m.* FROM sys_menu m, sys_roles_perms r, sys_permission  WHERE " +
            "r.role_id IN(?1) AND type != ?2 AND sys_permission.perm_id=r.perm_id AND (sys_permission.code=m.permission OR m.permission IS NULL) order by m.menu_sort asc",nativeQuery = true)
    LinkedHashSet<Menu> findByRoleIdsAndTypeNot(Set<Long> roleIds, int type);

    @Query(value = "SELECT m.* FROM sys_menu m WHERE type != ?1 order by m.menu_sort asc; " ,nativeQuery = true)
    LinkedHashSet<Menu> findByAdmin(int type);
