...
export default {
  ...
  cruds() {
    return CRUD({ title: '角色', url: 'api-user/roles', sort: 'level,asc', crudMethod: { ...crudRoles }})
  },
  ...
  methods: {
    ...
    menuChange(menu) {
      // 获取该节点的所有子节点，id 包含自身
      const _this = this
      getMenusTree(menu.id).then(res => {
        // 判断是否在 menuIds 中，如果存在则删除，否则添加
        if (_this.menuIds.indexOf(menu.id) !== -1) {
          for (let i = 0; i < res.length; i++) {
            const index = _this.menuIds.indexOf(res[i])
            if (index !== -1) {
              _this.menuIds.splice(index, 1)
            }
          }
        } else {
          for (let i = 0; i < res.length; i++) {
            const index = this.menuIds.indexOf(res[i].id)
            if (index === -1) {
              _this.menuIds.push(res[i].id)
            }
          }
        }
      })
